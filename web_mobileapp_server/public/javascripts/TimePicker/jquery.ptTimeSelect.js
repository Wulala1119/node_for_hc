...
(function($){
    ...
    jQuery.ptTimeSelect.setHr = function(h) {
        ...
        if (    h.toLowerCase() == "am"
            ...
            if(h>'12')
            {
                jQuery('#ptTimeSelectUserSelAmPm').empty().append("pm")
            }
    ...
    jQuery.ptTimeSelect.setTime = function() {
        var hour;
        var min;
        hour = parseInt(jQuery('#ptTimeSelectUserSelHr').text());
        min = jQuery('#ptTimeSelectUserSelMin').text();
        if(jQuery('#ptTimeSelectUserSelAmPm').text()=="PM")
        {
            hour += 12;
        }
        var tSel = String(hour) + ":" + min;
            /*
    ...
    jQuery.ptTimeSelect.openCntr = function (ele) {
        ...
        /////////////
        var old_time = String(jQuery(".isPtTimeSelectActive").val());
        var m_index = old_time.indexOf(":");
        var old_hour = old_time.substring(0,m_index);
        var old_min = old_time.substring(m_index+1);
        /////////////
        jQuery(ele).keyup(function(event){
            jQuery.fn.updateFromInput(event);
        });

        var hr    = old_hour > 12 ? old_hour - 12 : old_hour;//1;
        var min   = old_min;//'00';
        var tm    = old_hour > 12 ? 'PM' : 'AM';
        ...
        /*if (i.val()) {
            ...
            if (match) {
                ...
                //min    = match[2] || '00';
                //tm    = match[3] || 'AM';
                hr = match[1] > 12 ? match[1] - 12 : match[1];
                min = match[2];
                tm = match[1] > 12 ? 'PM' : 'AM';
            }
        }*/
    ...
    jQuery.ptTimeSelect._doCheckMouseClick = function(ev){
        ...
        if (   !jQuery(ev.target).closest("#ptTimeSelectCntr").length
            ...
            jQuery.ptTimeSelect.setTime();          //setTime when click somewhere else
            //jQuery.ptTimeSelect.closeCntr();
        }
    ...
    jQuery.fn.updateFromInput = function (event) {

        var temp = String(jQuery(".isPtTimeSelectActive").val());
        var time = temp;
        ...
        {
            case 0:break;
            case 1:if( temp.charAt(last)>'2' || temp.charAt(last)<'0' ){ time = ''; }break;
            case 2:if(
                        ( temp.charAt(last-1)=='2'&(temp.charAt(last)>'3'||temp.charAt(last)<'0') )     //limit 00-23
                        || ( (temp.charAt(last-1)>'1'||temp.charAt(last-1)<'0')&(temp.charAt(last)>'9'||temp.charAt(last)<'0'))
                     )
                        { time = temp.substring(0,last); }
                    else if(event.keyCode != 8)
                        {time += ":";}break;
            case 4:if( temp.charAt(last)>'5' || temp.charAt(last)<'0' ){ time = temp.substring(0,last); }break; //limit 00-59
            case 5:if( temp.charAt(last)>'9' || temp.charAt(last)<'0' ){ time = temp.substring(0,last); }break;
            default:time = temp.substring(0,5); break;
        }
        //console.log(time);
        jQuery(".isPtTimeSelectActive").val(time);
        var m_index = time.indexOf(":");
        var hour = time.substring(0,m_index);
        var min = time.substring(m_index+1);
        jQuery.ptTimeSelect.setMin(min);
        jQuery.ptTimeSelect.setHr(hour);
    };
})(jQuery);
